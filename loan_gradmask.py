...
def grad_mask_loan(helper, local_model, target_model, ratio=0.5):
    ...
    for poison_id in helper.params['adversary_list']:
        ...
        if count > 11:
            break
        train_data = helper.statehelper_dic[poison_id].get_trainloader()
        data_iterator = train_data
        for batch_id, (datas, labels) in enumerate(data_iterator):
            input = datas. float().cuda()
        ...
        for epoch in range(start_epoch, start_epoch + helper.params['aggr_epoch_interval']):
            ...
            if is_poison and state_key in helper.params['adversary_list'] and (epoch in localmodel_poison_epochs):
                ...
                if helper.params['gradmask_ratio'] != 1:
                    ...
                    if not IsGrad_mask:
                        mask_grad_list = grad_mask_loan(helper, local_model, target_model,
