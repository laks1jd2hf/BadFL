...
import copy
import config
from image_trigger import cifar_trigger
import numpy as np


def grad_mask_cifar(helper, local_model, target_model, ratio=0.5):
        ...
        for epoch in range(start_epoch, start_epoch + helper.params['aggr_epoch_interval']):
            ...
            if is_poison and agent_name_key in helper.params['adversary_list'] and (epoch in localmodel_poison_epochs):
                ...
                if helper.params['gradmask_ratio'] != 1:
                    ...
                    if not IsGrad_mask:
                        mask_grad_list = grad_mask_cifar(helper, local_model, target_model,
