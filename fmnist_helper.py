...
import numpy as np
from model.FMnistNet import FMnistNet
...
class FMnistHelper(Helper):

    def create_model(self):
        local_model = None
        target_model = None

        local_model = FMnistNet(name='Local', created_time=self.params['current_time'])
        target_model = FMnistNet(name='Target', created_time=self.params['current_time'])
    ...
    def sample_dirichlet_train_data(self, no_participants, alpha=0.9):
        ...
        class_size = len(cifar_classes[0])  # for cifar: 5000   每一类数据的样本数
...
if __name__ == '__main__':
    np.random.seed(1)
    with open(f'./utils/mnist_params.yaml', 'r') as f:
